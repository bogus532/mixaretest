...
public class PaintScreen {
	...
	public void paintObj(ScreenObj obj, float x, float y, float rotation,
		...
		//Log.d("Mixare",x+", "+y+", "+obj.getWidth()/2+", "+obj.getHeight()/2+" , "+scale);
	}
	
	//bogus
	public void paintObj2(ScreenObj obj, float x, float y, float rotation,
			float scale) {
		canvas.save();
		canvas.translate(x + obj.getWidth() / 2, y + obj.getHeight() / 2);
		canvas.rotate(rotation);
		...
		canvas.restore();
		//Log.d("MixView","paintObj2, w : "+obj.getWidth()/2+", h : "+obj.getHeight()/2+", x: "+x+", y : "+y);	
		//Log.d("Mixare","rotation = " + rotation);
	}
