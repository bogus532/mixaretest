...
import org.mixare.gui.RadarPoi;
...
public class DataView {
	...
	public void init(int widthInit, int heightInit) {
		try {
			...
			//bogus
			rx = width/2-1-RadarPoints.RADIUS;
			ry = (height/2-1)-RadarPoints.RADIUS;
			Log.d("MixView","width : "+width+", height : "+height+", rx = "+rx+", ry = "+ry);
	...
	public void draw(PaintScreen dw) {
		...
		if (state.nextLStatus == MixState.NOT_STARTED && !frozen) {
			...
			if (mixContext.getStartUrl().length() > 0){
				...
				//bogus
				Log.d("MixView","alt = "+curFix.getAltitude());
				//double lat = curFix.getLatitude(), lon = curFix.getLongitude(),alt = 0;
			...
			while((dRes=dm.getNextResult())!=null)
			{
				if (dRes.error && retry < 3) {
					...
					Log.d("MixView",mixContext.getResources().getString(R.string.download_error) +" "+ dRes.errorRequest.url);
				...
				if(!dRes.error) {
					...
					Log.d("MixView",mixContext.getResources().getString(R.string.download_received) +" "+ dRes.source);
		...
		for (int i = dataHandler.getMarkerCount()-1; i >= 0; i--) {
			...
			if (ma.isActive() && (ma.getDistance() / 1000f < radius)) {
				...
				if(!frozen)
				{
					...
					if(addX != 0.0)
					{
						Log.d("MixView","addX= "+addX+", addY= "+addY);
					}
				}
				ma.draw(dw);
		...
		//bogus
		radarPoi.view = this;
		dw.paintObj2(radarPoi, rx, ry, -state.getCurBearing(), 1); 
		//dw.paintRect(rx, ry, 10, 10);
		//float tx = 0; //39;
		//float ty = 0; //199;
		//dw.paintRect((float)196.37411+tx, (float)114.64959+ty, 20, 10);
		//dw.paintRect((float)154.3933, (float)193.56528, 20, 10);
		//dw.paintRect((float)156.05171, (float)6.469475, 20, 10);
		
		//dw.paintRect((float)207.12393+tx, (float)187.50148+ty, 20, 20);
		//dw.paintRect((float)165.1431+tx, (float)266.41718+ty, 20, 20);
		//dw.paintRect((float)166.12393+tx, (float)79.32137+ty, 20, 20);
        ...
        //bogus
        radarText(dw, MixUtils.formatDist(radius*2/3 * 1000), rx + RadarPoints.RADIUS, ry + RadarPoints.RADIUS*5/3 -10, false);
		radarText(dw, MixUtils.formatDist(radius/3 * 1000), rx + RadarPoints.RADIUS, ry + RadarPoints.RADIUS*4/3 -10, false);
		...
		if (evt != null) {
			switch (evt.type) {
				case UIEvent.KEY:	handleKeyEvent((KeyEvents) evt);		break;
				//case UIEvent.CLICK:	handleClickEvent((ClickEvent) evt);	break;
				case UIEvent.CLICK:	handleClickEvent2((ClickEvent) evt,-state.getCurBearing());	break;
			}
		}
		state.nextLStatus = MixState.PROCESSING;				
	}
	//bogus remove radar
	public void draw2(PaintScreen dw) {
		mixContext.getRM(cam.transform);
		curFix = mixContext.getCurrentLocation();
	...
	private void handleKeyEvent(KeyEvents evt) {
	...
	boolean handleClickEvent2(ClickEvent evt,float rotation) {
		boolean evtHandled = false;
		float curX = 0;
		float curY = 0;
		...
		Log.d("MixView","evt.x = "+evt.x+" ,evt.y = "+evt.y);
		//Log.d("fClick2","-----------------------------------------------");
		// Handle event
		if (state.nextLStatus == MixState.DONE) {
			//the following will traverse the markers in ascending order (by distance) the first marker that 
			...
			for (int i = 0 ; i < dataHandler.getMarkerCount() && !evtHandled; i++) {
				Marker pm = dataHandler.getMarker(i);
				//Log.d("DataView",i+": x = "+pm.getOriginVector().x+", y = "+pm.getOriginVector().z);
				curX = (pm.getOriginVector().x )+addX;
				curY = (pm.getOriginVector().z )+addY;
				pm.setCenter(rx+RadarPoints.RADIUS, ry+RadarPoints.RADIUS);
				//bogus
				evtHandled = pm.fClick(evt.x, evt.y, mixContext, state,curX,curY,rotation,barHeight);
			}
		}
		return evtHandled;
	}
	...
	public void keyEvent(int keyCode) {
		synchronized (uiEvents) {
			uiEvents.add(new KeyEvents(keyCode));
		}
	}
...
class KeyEvents extends UIEvent {
	public int keyCode;

	public KeyEvents(int keyCode) {
		this.type = KEY;
